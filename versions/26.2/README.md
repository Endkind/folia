...
This Docker image provides Folia 26.2 Minecraft Server. You can easily run a Minecraft server using this image.

## Quick start

```bash
docker run -it -d -p 25565:25565 --name endkind-folia endkind/folia:26.2
```
...
docker run -it -d -p 25565:25565 --name endkind-folia -v endkind-folia:/data -e MAX_RAM=3G --restart=always endkind/folia:26.2
```
...
```bash
docker build -t endkind/folia:26.2 .
```
