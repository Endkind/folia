...
This Docker image provides Folia 1.21.11 Minecraft Server. You can easily run a Minecraft server using this image.

## Quick start

```bash
docker run -it -d -p 25565:25565 --name endkind-folia endkind/folia:1.21.11
```
...
docker run -it -d -p 25565:25565 --name endkind-folia -v endkind-folia:/data -e MAX_RAM=3G --restart=always endkind/folia:1.21.11
```
...
```bash
docker build -t endkind/folia:1.21.11 .
```
